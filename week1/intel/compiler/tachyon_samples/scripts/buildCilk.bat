@echo off
echo Building build_with_cilk project 

if ""%1"" == ""clean"" goto clean

rem for building win32 release config only
rem build "build_with_cilk" program
...
:clean
echo cleaning Intel(R) cilk(TM) build files
...
:build_solution
echo Building Intel(R) cilk(TM) Solution
set CFLAGS=/c /O2 /Oi /Ot /Qipo /EHsc /MD /GS /arch:SSE2 /fp:fast /W2 /Zi /I src
set DefFlags=/D "WIN32" /D "_WINDOWS" /D "NDEBUG" /D "_MBCS" 
set OutDir=.\tachyon_release\\
set OutFlags=/Fo"%OutDir%" 
set outFName=tachyon.cilk_solution
set outFDir=tachyon.cilk
